    ...
    { float tf_a = buf_transfers[1*7+0];
      if (tf_a != 0.0) {
        ...
        for (int ch = 0; ch < 3; ch++) {
          float xv = sc52[ch];
          sc52[ch] = xv >= tf_d ? tf_a * pow(xv, 1.0 / tf_g) + tf_b : tf_c * xv + tf_f;
        }
      }
    }
    switch (buf_fmts[1]) {
      ...
      case 5u: ((device half*)(p1 + y * buf_rbs[1]))[x] = half(sc52.x); break;
      case 6u: ((device float*)(p1 + y * buf_rbs[1]))[x] = sc52.x; break;
      case 7u: { ((device half*)(p1 + y * buf_rbs[1]))[x] = half(sc52.x); ((device half*)(p1_g + y * buf_rbs[1]))[x] = half(sc52.y); ((device half*)(p1_b + y * buf_rbs[1]))[x] = half(sc52.z); ((device half*)(p1_a + y * buf_rbs[1]))[x] = half(sc52.w); break; }
      default: break;
    }
}
