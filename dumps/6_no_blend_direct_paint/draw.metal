...
kernel void umbra_entry(
    ...
    if (planes_p1 == 1) {
        ...
        v22_c = unpack_unorm4x8_srgb_to_float(((device uint*)(p1 + y * buf_rbs[1]))[x]);
    }
    float v22 = v22_c.x;
    float v22_1 = v22_c.y;
    float v22_2 = v22_c.z;
    float v22_3 = v22_c.w;
    float v23 = as_type<float>(v1) - v22;
    float v24 = fma(as_type<float>(v21), v23, v22);
    float v25 = as_type<float>(v2) - v22_1;
    float v26 = fma(as_type<float>(v21), v25, v22_1);
    float v27 = as_type<float>(v3) - v22_2;
    float v28 = fma(as_type<float>(v21), v27, v22_2);
    float v29 = as_type<float>(v4) - v22_3;
    float v30 = fma(as_type<float>(v21), v29, v22_3);
    float4 sc31 = float4(v24, v26, v28, v30);
    if (planes_p1 == 1) {
        float4 tw31 = (fmt_p1 == 3u || fmt_p1 == 4u) ? float4(half4(sc31)) : sc31;
        tex_p1_0.write(tw31, uint2(x,y));
    } else if (planes_p1 == 4) {
        half4 tw31 = half4(sc31);
        tex_p1_0.write(float4(tw31.x,0,0,0), uint2(x,y));
        tex_p1_1.write(float4(tw31.y,0,0,0), uint2(x,y));
        tex_p1_2.write(float4(tw31.z,0,0,0), uint2(x,y));
        tex_p1_3.write(float4(tw31.w,0,0,0), uint2(x,y));
    } else if (fmt_p1 == 0u) {
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm4x8(clamp(sc31, 0.0, 1.0));
    } else if (fmt_p1 == 1u) {
        ((device ushort*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm565(clamp(sc31.zyx, 0.0, 1.0));
    } else if (fmt_p1 == 2u) {
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm10a2(clamp(sc31, 0.0, 1.0));
    } else if (fmt_p1 == 3u) {
        device half *hp = (device half*)(p1 + y * buf_rbs[1]) + x*4;
        ...
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_srgb_unorm4x8(clamp(sc31, 0.0, 1.0));
    }
}
