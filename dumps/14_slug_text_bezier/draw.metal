...
kernel void umbra_entry(
    ...
    if (planes_p1 == 1) {
        ...
        v11_c = float4(bgr.z, bgr.y, bgr.x, 1.0);
    } else if (fmt_p1 == 2u) {
        ...
        v11_c = unpack_unorm4x8_srgb_to_float(((device uint*)(p1 + y * buf_rbs[1]))[x]);
    }
    float v11 = v11_c.x;
    float v11_1 = v11_c.y;
    float v11_2 = v11_c.z;
    ...
    if (planes_p1 == 1) {
        float4 tw24 = (fmt_p1 == 3u || fmt_p1 == 4u) ? float4(half4(sc24)) : sc24;
        tex_p1_0.write(tw24, uint2(x,y));
    } else if (planes_p1 == 4) {
        half4 tw24 = half4(sc24);
        tex_p1_0.write(float4(tw24.x,0,0,0), uint2(x,y));
        tex_p1_1.write(float4(tw24.y,0,0,0), uint2(x,y));
        tex_p1_2.write(float4(tw24.z,0,0,0), uint2(x,y));
        tex_p1_3.write(float4(tw24.w,0,0,0), uint2(x,y));
    } else if (fmt_p1 == 0u) {
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm4x8(clamp(sc24, 0.0, 1.0));
    } else if (fmt_p1 == 1u) {
        ((device ushort*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm565(clamp(sc24.zyx, 0.0, 1.0));
    } else if (fmt_p1 == 2u) {
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm10a2(clamp(sc24, 0.0, 1.0));
    } else if (fmt_p1 == 3u) {
        device half *hp = (device half*)(p1 + y * buf_rbs[1]) + x*4;
        ...
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_srgb_unorm4x8(clamp(sc24, 0.0, 1.0));
    }
}
