...
kernel void umbra_entry(
    ...
    if (planes_p1 == 1) {
        ...
        v7_c = float4(bgr.z, bgr.y, bgr.x, 1.0);
    } else if (fmt_p1 == 2u) {
    ...
    if (planes_p1 == 1) {
        float4 tw12 = (fmt_p1 == 3u || fmt_p1 == 4u) ? float4(half4(sc12)) : sc12;
        tex_p1_0.write(tw12, uint2(x,y));
    } else if (planes_p1 == 4) {
        half4 tw12 = half4(sc12);
        tex_p1_0.write(float4(tw12.x,0,0,0), uint2(x,y));
        tex_p1_1.write(float4(tw12.y,0,0,0), uint2(x,y));
        tex_p1_2.write(float4(tw12.z,0,0,0), uint2(x,y));
        tex_p1_3.write(float4(tw12.w,0,0,0), uint2(x,y));
    } else if (fmt_p1 == 0u) {
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm4x8(clamp(sc12, 0.0, 1.0));
    } else if (fmt_p1 == 1u) {
        ((device ushort*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm565(clamp(sc12.zyx, 0.0, 1.0));
    } else if (fmt_p1 == 2u) {
        ((device uint*)(p1 + y * buf_rbs[1]))[x] = pack_float_to_unorm10a2(clamp(sc12, 0.0, 1.0));
    } else if (fmt_p1 == 3u) {
        device half *hp = (device half*)(p1 + y * buf_rbs[1]) + x*4;
