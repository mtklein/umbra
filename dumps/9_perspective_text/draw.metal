...
kernel void umbra_entry(
    ...
    if (planes_p1 == 1) {
        float4 tw71 = (fmt_p1 == 3u || fmt_p1 == 4u) ? float4(half4(sc71)) : sc71;
        tex_p1_0.write(tw71, uint2(x,y));
    } else if (planes_p1 == 4) {
        half4 tw71 = half4(sc71);
        tex_p1_0.write(float4(tw71.x,0,0,0), uint2(x,y));
        tex_p1_1.write(float4(tw71.y,0,0,0), uint2(x,y));
        tex_p1_2.write(float4(tw71.z,0,0,0), uint2(x,y));
        tex_p1_3.write(float4(tw71.w,0,0,0), uint2(x,y));
    } else if (fmt_p1 == 0u) {
