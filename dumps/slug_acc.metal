#include <metal_stdlib>
using namespace metal;

static inline int clamp_ix(int ix, uint bytes, int elem) {
    int hi = (int)(bytes / (uint)elem) - 1;
    if (hi < 0) hi = 0;
    return clamp(ix, 0, hi);
}

kernel void umbra_entry(
    constant uint &n [[buffer(0)]],
    device uchar *p0 [[buffer(1)]],
    device uchar *p1 [[buffer(2)]],
    device uchar *p2 [[buffer(3)]],
    constant uint *buf_szs [[buffer(4)]],
    uint i [[thread_position_in_grid]]
) {
    if (i >= n) return;
    uint v0 = 0u;
    uint v2 = ((device const uint*)p1)[0];
    uint v3 = 1u;
    uint v4 = ((device const uint*)p1)[1];
    uint v5 = as_type<uint>((float)(int)v4);
    uint v6 = 2u;
    uint v7 = ((device const uint*)p1)[2];
    uint v8 = 3u;
    uint v9 = ((device const uint*)p1)[3];
    uint v10 = 4u;
    uint v11 = ((device const uint*)p1)[4];
    uint v12 = 5u;
    uint v13 = ((device const uint*)p1)[5];
    uint v14 = 6u;
    uint v15 = ((device const uint*)p1)[6];
    uint v16 = ((device const uint*)p1)[7];
    uint v17 = ((device const uint*)p1)[8];
    uint v18 = ((device const uint*)p1)[9];
    uint v19 = ((device const uint*)p1)[10];
    uint v20 = ((device const uint*)p1)[11];
    uint v21 = ((device const uint*)p1)[12];
    uint v22 = as_type<uint>(as_type<float>(v5) * as_type<float>(v18));
    uint v23 = as_type<uint>(as_type<float>(v5) * as_type<float>(v9));
    uint v24 = as_type<uint>(as_type<float>(v5) * as_type<float>(v15));
    uint v25 = 1065353216u;
    uint v26 = 1073741824u;
    uint v27 = 931135488u;
    uint v28 = ((device const uint*)p1)[18];
    uint v29 = v14 * v28;
    uint v30 = ((device const uint*)p2)[v29];
    uint v31 = v3 + v29;
    uint v32 = ((device const uint*)p2)[v31];
    uint v33 = v6 + v29;
    uint v34 = ((device const uint*)p2)[v33];
    uint v35 = v8 + v29;
    uint v36 = ((device const uint*)p2)[v35];
    uint v37 = v10 + v29;
    uint v38 = ((device const uint*)p2)[v37];
    uint v39 = v12 + v29;
    uint v40 = ((device const uint*)p2)[v39];
    uint v41 = 3212836864u;
    uint v42 = i;
    uint v43 = v42 + v2;
    uint v44 = as_type<uint>((float)(int)v43);
    uint v45 = as_type<uint>(fma(as_type<float>(v44), as_type<float>(v13), as_type<float>(v24)));
    uint v46 = as_type<uint>(fma(as_type<float>(v44), as_type<float>(v17), as_type<float>(v22)));
    uint v47 = as_type<uint>(fma(as_type<float>(v44), as_type<float>(v7), as_type<float>(v23)));
    uint v48 = as_type<uint>(as_type<float>(v11) + as_type<float>(v47));
    uint v49 = as_type<uint>(as_type<float>(v16) + as_type<float>(v45));
    uint v50 = as_type<uint>(as_type<float>(v19) + as_type<float>(v46));
    uint v51 = as_type<uint>(as_type<float>(v49) / as_type<float>(v50));
    uint v52 = as_type<uint>(as_type<float>(v40) - as_type<float>(v51));
    uint v53 = as_type<float>(v51) <  as_type<float>(v21) ? 0xffffffffu : 0u;
    uint v54 = as_type<uint>(as_type<float>(v36) - as_type<float>(v51));
    uint v55 = as_type<uint>(as_type<float>(v32) - as_type<float>(v51));
    uint v56 = as_type<uint>(as_type<float>(v48) / as_type<float>(v50));
    uint v57 = as_type<uint>(as_type<float>(v38) - as_type<float>(v56));
    uint v58 = as_type<float>(v56) <  as_type<float>(v20) ? 0xffffffffu : 0u;
    uint v59 = as_type<uint>(as_type<float>(v34) - as_type<float>(v56));
    uint v60 = as_type<uint>(as_type<float>(v30) - as_type<float>(v56));
    uint v61 = as_type<uint>(as_type<float>(v59) - as_type<float>(v60));
    uint v62 = as_type<uint>(as_type<float>(v26) * as_type<float>(v61));
    uint v63 = as_type<uint>(as_type<float>(v55) - as_type<float>(v54));
    uint v64 = as_type<float>(v0) <= as_type<float>(v56) ? 0xffffffffu : 0u;
    uint v65 = v64 & v58;
    uint v66 = as_type<float>(v0) <= as_type<float>(v51) ? 0xffffffffu : 0u;
    uint v67 = v66 & v53;
    uint v68 = v65 & v67;
    uint v69 = as_type<uint>(as_type<float>(v26) * as_type<float>(v54));
    uint v70 = as_type<uint>(as_type<float>(v55) - as_type<float>(v69));
    uint v71 = as_type<uint>(as_type<float>(v52) + as_type<float>(v70));
    uint v72 = as_type<uint>(as_type<float>(v63) * as_type<float>(v63));
    uint v73 = as_type<uint>(as_type<float>(v72) - as_type<float>(v55) * as_type<float>(v71));
    uint v74 = as_type<uint>(max(as_type<float>(v0), as_type<float>(v73)));
    uint v75 = as_type<uint>(sqrt(as_type<float>(v74)));
    uint v76 = as_type<uint>(as_type<float>(v63) + as_type<float>(v75));
    uint v77 = as_type<uint>(as_type<float>(v0) - as_type<float>(v71));
    uint v78 = as_type<uint>(max(as_type<float>(v71), as_type<float>(v77)));
    uint v79 = as_type<float>(v27) <  as_type<float>(v78) ? 0xffffffffu : 0u;
    uint v80 = (v79 & v71) | (~v79 & v25);
    uint v81 = as_type<uint>(as_type<float>(v25) / as_type<float>(v80));
    uint v82 = as_type<uint>(as_type<float>(v81) * as_type<float>(v76));
    uint v83 = as_type<uint>(as_type<float>(v71) * as_type<float>(v82));
    uint v84 = as_type<uint>(as_type<float>(v83) - as_type<float>(v63));
    uint v85 = as_type<uint>(as_type<float>(v63) - as_type<float>(v75));
    uint v86 = as_type<uint>(as_type<float>(v81) * as_type<float>(v85));
    uint v87 = as_type<uint>(as_type<float>(v0) - as_type<float>(v63));
    uint v88 = as_type<uint>(max(as_type<float>(v63), as_type<float>(v87)));
    uint v89 = as_type<float>(v27) <  as_type<float>(v88) ? 0xffffffffu : 0u;
    uint v90 = as_type<uint>(as_type<float>(v26) * as_type<float>(v63));
    uint v91 = (v89 & v90) | (~v89 & v25);
    uint v92 = as_type<uint>(as_type<float>(v55) / as_type<float>(v91));
    uint v93 = (v79 & v86) | (~v79 & v92);
    uint v94 = as_type<uint>(as_type<float>(v71) * as_type<float>(v93));
    uint v95 = as_type<uint>(as_type<float>(v94) - as_type<float>(v63));
    uint v96 = as_type<float>(v93) <  as_type<float>(v25) ? 0xffffffffu : 0u;
    uint v97 = as_type<float>(v0) <= as_type<float>(v93) ? 0xffffffffu : 0u;
    uint v98 = v97 & v96;
    uint v99 = as_type<float>(v0) <= as_type<float>(v73) ? 0xffffffffu : 0u;
    uint v100 = v99 & v79;
    uint v101 = v99 & v98;
    uint v102 = as_type<float>(v82) <  as_type<float>(v25) ? 0xffffffffu : 0u;
    uint v103 = as_type<float>(v0) <= as_type<float>(v82) ? 0xffffffffu : 0u;
    uint v104 = v103 & v102;
    uint v105 = v100 & v104;
    uint v106 = as_type<uint>(as_type<float>(v26) * as_type<float>(v59));
    uint v107 = as_type<uint>(as_type<float>(v60) - as_type<float>(v106));
    uint v108 = as_type<uint>(as_type<float>(v57) + as_type<float>(v107));
    uint v109 = as_type<uint>(fma(as_type<float>(v82), as_type<float>(v108), as_type<float>(v62)));
    uint v110 = as_type<uint>(fma(as_type<float>(v82), as_type<float>(v109), as_type<float>(v60)));
    uint v111 = as_type<float>(v0) <  as_type<float>(v110) ? 0xffffffffu : 0u;
    uint v112 = v105 & v111;
    uint v113 = as_type<uint>(fma(as_type<float>(v93), as_type<float>(v108), as_type<float>(v62)));
    uint v114 = as_type<uint>(fma(as_type<float>(v93), as_type<float>(v113), as_type<float>(v60)));
    uint v115 = as_type<float>(v0) <  as_type<float>(v114) ? 0xffffffffu : 0u;
    uint v116 = v101 & v115;
    uint v117 = as_type<float>(v95) <  as_type<float>(v0) ? 0xffffffffu : 0u;
    uint v118 = (v117 & v41) | (~v117 & v0);
    uint v119 = as_type<float>(v0) <  as_type<float>(v95) ? 0xffffffffu : 0u;
    uint v120 = (v119 & v25) | (~v119 & v118);
    uint v121 = (v116 & v120) | (~v116 & v0);
    uint v122 = as_type<float>(v84) <  as_type<float>(v0) ? 0xffffffffu : 0u;
    uint v123 = (v122 & v41) | (~v122 & v0);
    uint v124 = as_type<float>(v0) <  as_type<float>(v84) ? 0xffffffffu : 0u;
    uint v125 = (v124 & v25) | (~v124 & v123);
    uint v126 = (v112 & v125) | (~v112 & v0);
    uint v127 = as_type<uint>(as_type<float>(v121) + as_type<float>(v126));
    uint v128 = (v68 & v127) | (~v68 & v0);
    uint v129 = ((device uint*)p0)[i];
    uint v130 = as_type<uint>(as_type<float>(v128) + as_type<float>(v129));
    ((device uint*)p0)[i] = v130;
}
