...
static int cmp_double(void const *a,
                      void const *b) {
    double const x = *(double const*)a;
    double const y = *(double const*)b;
    return (x > y) - (x < y);
}
...
static void run_mtl(void *ctx, int n,
                    umbra_buf buf[]) {
...
static jit_entry_fn incbin_load(
        struct umbra_jit *jit, const char *sym) {
    char bin_path[128], s_path[128];
    char dylib_path[128], cmd[512];
    snprintf(bin_path, sizeof bin_path,
             "/tmp/umbra_%s.bin", sym);
    snprintf(s_path, sizeof s_path,
             "/tmp/umbra_%s.S", sym);
    snprintf(dylib_path, sizeof dylib_path,
             "/tmp/umbra_%s.dylib", sym);
...
static void run_incbin(void *ctx, int n,
                       umbra_buf buf[]) {
    (void)ctx;
    incbin_fn_ctx(n, buf);
}
...
static double bench_run(run_fn fn, void *ctx,
                        int pixel_n,
                        umbra_buf buf[]) {
    fn(ctx, pixel_n, buf);
...
static void build_optimize_only(
        struct umbra_basic_block *bb) {
    umbra_basic_block_free(bb);
}
static void build_interp(
        struct umbra_basic_block *bb) {
...
static void fmt_ns(char buf[16], double ns) {
    if      (ns >= 1e6) {
        sprintf(buf, "%7.1f ms", ns * 1e-6);
    } else if (ns >= 1e3) {
        sprintf(buf, "%7.1f us", ns * 1e-3);
    } else {
        sprintf(buf, "%7.0f ns", ns);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (incbin && jit) {
        srcover_incbin =
            incbin_load(jit, "srcover");
        if (!srcover_incbin) {
            fprintf(stderr,
                "warning: incbin load failed"
                " for srcover\n");
        }
    }
#endif
    ...
    uint32_t *src =
        malloc((size_t)pixel_n * sizeof *src);
    __fp16 *dr =
        malloc((size_t)pixel_n * sizeof *dr);
    __fp16 *dg =
        malloc((size_t)pixel_n * sizeof *dg);
    __fp16 *db =
        malloc((size_t)pixel_n * sizeof *db);
    __fp16 *da =
        malloc((size_t)pixel_n * sizeof *da);
    ...
    if (samples <= 1) {
        ...
        fmt_ns(build_buf, bench_build(
            build_srcover, build_optimize_only));
        printf("  optimize%s\n", build_buf);

        fmt_ns(build_buf, bench_build(
            build_srcover, build_interp));
        sprintf(run_buf, "%5.2f ns/px",
            bench_run(run_interp, p,
                      pixel_n, buf));
        printf("  interp  %s  %s\n",
               build_buf, run_buf);
        ...
        if (mtl) {
            fmt_ns(build_buf, bench_build(
                build_srcover, build_metal));
            sprintf(run_buf, "%5.2f ns/px",
                bench_run(run_mtl, mtl,
                          pixel_n, buf));
            printf("  metal   %s  %s\n",
                   build_buf, run_buf);
        }
        ...
        double *s =
            malloc((size_t)samples * sizeof *s);

        printf("             min     median"
               "     max\n");

        for (int i = 0; i < samples; i++) {
            s[i] = bench_run(run_interp, p,
                             pixel_n, buf);
        }
        qsort(s, (size_t)samples,
              sizeof *s, cmp_double);
        printf("  interp    %5.2f     %5.2f"
               "    %5.2f  ns/px\n",
        ...
        if (srcover_incbin) {
            incbin_fn_ctx = srcover_incbin;
            for (int i = 0; i < samples; i++) {
                s[i] = bench_run(run_incbin, NULL,
                                 pixel_n, buf);
            }
            qsort(s, (size_t)samples,
                  sizeof *s, cmp_double);
            printf("  incbin    %5.2f     %5.2f"
                   "    %5.2f  ns/px\n",
                   s[0], s[samples/2],
                   s[samples-1]);
        }
        ...
        if (mtl) {
            for (int i = 0; i < samples; i++) {
                s[i] = bench_run(run_mtl, mtl,
                                 pixel_n, buf);
            }
            qsort(s, (size_t)samples,
                  sizeof *s, cmp_double);
            printf("  metal     %5.2f     %5.2f"
                   "    %5.2f  ns/px\n",
                   s[0], s[samples/2],
                   s[samples-1]);
        }
    ...
    free(src);
    free(dr); free(dg); free(db); free(da);
    ...
    int ndraws =
        (int)(sizeof draws / sizeof draws[0]);
    for (int di = 0; di < ndraws; di++) {
        ...
        int px_bytes =
            (d->load == umbra_load_fp16) ? 8 : 4;
        void *dst_px = malloc(
            (size_t)pixel_n * (size_t)px_bytes);
        memset(dst_px, 0x80,
            (size_t)pixel_n * (size_t)px_bytes);
