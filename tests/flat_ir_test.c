...
TEST(test_dispatch_pair_alias) {
    ...
    for (int bi = 0; bi < NUM_BACKENDS; bi++) {
        ...
        (struct umbra_buf[]){{.ptr=in, .count=16},
            ...
            B.p[0]->dispatch(B.p[0], 0, 0, 16, 1, NULL, 0);
...
TEST(test_program_threadsafe) {
    // Early-bound store: two threads calling dispatch() share the bound buf, so
    // dispatch_is_threadsafe must be false on every backend regardless of whether
    ...
    // Late-bound store: dst is supplied per dispatch() call, so two threads can
    ...
    for (int i = 0; i < count(cases); i++) {
        ...
        p_early->dispatch_is_threadsafe == 0 here;
        umbra_program_free(p_early);

        struct umbra_program *p_late  = be->compile(be, late_ir);
        p_late->dispatch_is_threadsafe == want_tsafe here;
...
static void run_umbra_uniforms_test(struct umbra_backend *be) {
    ...
    // can be filled (and later mutated) any time before a dispatch() call.
    uint32_t u[4] = {0};
...
TEST(test_program_null_guards) {
    ...
    // dispatch with w=0 and h=0
    int32_t buf[1] = {0};
    slot[0] = (struct umbra_buf){.ptr=buf, .count=1};
    p->dispatch(p, 0, 0, 0, 1, NULL, 0);
    p->dispatch(p, 0, 0, 1, 0, NULL, 0);
...
TEST(test_strided_2d_dispatch) {
    ...
    for (int bi = 0; bi < NUM_BACKENDS; bi++) {
        ...
        if (B.p[bi]) {
            ...
            B.p[bi]->dispatch(B.p[bi], L, T, R, BT, NULL, 0);
...
TEST(test_gather_partial_oob) {
    ...
    for (int bi = 0; bi < NUM_BACKENDS; bi++) {
        ...
        if (B.p[bi]) {
            ...
            B.p[bi]->dispatch(B.p[bi], 0, 0, 16, 1, NULL, 0);
            B.be[bi]->flush(B.be[bi]);
...
TEST(test_if_gather_16) {
    // Same shape as test_if_gather_32 — gather_16 also bounds-checks per
    // lane, so OOB indices on false lanes are safe.
    struct umbra_buf slot[20] = {0};
    struct umbra_builder *b = umbra_builder();
    ...
    umbra_if(b, cond); {
        umbra_val16 const got = umbra_gather_16(b, umbra_bind_buf(b, &slot[0]), ix);
        umbra_store_var32(b, v, umbra_i32_from_u16(b, got));
    } umbra_end_if(b);
    umbra_store_32(b, umbra_bind_buf(b, &slot[1]), umbra_load_var32(b, v));
...
// Pin each source op's intrinsic scope and the narrow propagation rule.
// Walks the finalized IR and finds the value-producing op of a known op
// kind, asserting its computed scope matches the expectation.
static void scope_of(struct umbra_flat_ir const *ir, enum op op, int8_t want) {
    int found = 0;
    for (int i = 0; i < ir->insts; i++) {
        if (ir->inst[i].op == op) {
            ir->inst[i].scope == want here;
            found = 1;
        }
    }
    found here;
...
TEST(test_scope_intrinsics) {
    struct umbra_buf slot[4] = {0};

    // op_imm_32 → COMPILE; op_uniform_32 → DISPATCH; op_y → ROW; op_x → LANE.
    {
        struct umbra_builder *b = umbra_builder();
        umbra_ptr const u = umbra_bind_uniforms(b, NULL, 4);
        umbra_val32 const v = umbra_add_f32(b,
            umbra_uniform_32(b, u, 0),
            umbra_imm_f32(b, 1.0f));
        umbra_store_32(b, umbra_bind_buf(b, &slot[0]),
            umbra_add_f32(b, v,
                umbra_add_f32(b,
                    umbra_f32_from_i32(b, umbra_y(b)),
                    umbra_f32_from_i32(b, umbra_x(b)))));
        struct umbra_flat_ir *ir = umbra_flat_ir(b);
        scope_of(ir, op_imm_32,     SCOPE_COMPILE);
        scope_of(ir, op_uniform_32, SCOPE_DISPATCH);
        scope_of(ir, op_y,          SCOPE_ROW);
        scope_of(ir, op_x,          SCOPE_LANE);
        umbra_flat_ir_free(ir);
        umbra_builder_free(b);
    }
    ...
    }
    {
        struct umbra_builder *b = umbra_builder();
        umbra_ptr const buf = umbra_bind_buf(b, &slot[1]);
        umbra_val32 const v = umbra_gather_32(b, buf, umbra_x(b));
        umbra_store_32(b, umbra_bind_buf(b, &slot[0]), v);
        struct umbra_flat_ir *ir = umbra_flat_ir(b);
        scope_of(ir, op_gather_32, SCOPE_LANE);
        umbra_flat_ir_free(ir);
        umbra_builder_free(b);
    }
}
