...
static void run_long_batch_no_oom(struct umbra_backend *be) {
    if (be) {
        ...
        umbra_ptr32 const cu = umbra_uniforms  (bld, color, count(color)),
...
static void run_tiled_writable_sync(struct umbra_backend *be) {
    ...
    umbra_ptr32 const dp = umbra_bind_buf32(b, &data_buf);
...
TEST(test_wgpu_misc) {
    ...
    struct umbra_builder *bld = umbra_builder();
    umbra_ptr32 const cu = umbra_uniforms  (bld, uniform_data, count(uniform_data)),
                      pp = umbra_bind_buf32(bld, &pixel_buf);
    umbra_color_val32 c = {
        umbra_uniform_32(bld, cu, 0),
