...
extern slide slide_solid(char const*, uint32_t,
                         float const[4],
                         umbra_coverage_fn,
                         umbra_blend_fn,
                         umbra_load_fn,
...
extern slide slide_gradient_2stop(char const*, uint32_t,
                                  umbra_shader_fn,
                                  umbra_store_fn,
                                  float const[8],
                                  float const[4]);
extern slide slide_gradient_lut(char const*, uint32_t,
                                umbra_shader_fn,
                                umbra_store_fn,
                                float const[4],
                                float*, int);
...
void slides_init(int w, int h) {
    ...
    all[count++] = slide_gradient_2stop(
        ...
        (float[]){1.0f,0.4f,0.0f,1.0f,
                  0.0f,0.3f,1.0f,1.0f},
        (float[]){1.0f/640.0f, 0.0f, 0.0f, 0.0f});
    all[count++] = slide_gradient_2stop(
        "11. Radial Gradient (2-stop)", 0xff000000,
        umbra_shader_radial_2, umbra_store_8888,
        (float[]){1.0f,1.0f,0.9f,1.0f,
                  0.05f,0.0f,0.15f,1.0f},
        (float[]){320.0f, 240.0f,
                  1.0f/300.0f, 0.0f});
    all[count++] = slide_gradient_lut(
        "12. Linear Gradient (wide gamut)",
        0xff000000,
        umbra_shader_linear_grad, umbra_store_8888,
        (float[]){1.0f/640.0f, 0.0f,
                  0.0f, 64.0f},
        linear_lut, LUT_N);
    all[count++] = slide_gradient_lut(
        "13. Radial Gradient (wide gamut)",
        0xff000000,
