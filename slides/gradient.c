...
static umbra_color_val32 walk_stops(struct umbra_builder *b, umbra_val32 t,
    ...
    umbra_var32 vr = umbra_declare_var32(b, zero_f),
                       vg = umbra_declare_var32(b, zero_f),
                       vb = umbra_declare_var32(b, zero_f),
                       va = umbra_declare_var32(b, zero_f);
...
umbra_color_val32 shader_gradient_evenly_spaced_stops(void *ctx, struct umbra_builder *b,
    ...
    umbra_val32 const t = self->coords_fn(self->coords_ctx, b,
                                          (umbra_point_val32){x, y});
    umbra_ptr const u      = umbra_bind_uniforms(b, self, sizeof *self / 4);
    umbra_ptr const colors = umbra_bind_sealed(b, &self->colors);
...
umbra_color_val32 shader_gradient(void *ctx, struct umbra_builder *b,
    ...
    umbra_ptr const u      = umbra_bind_uniforms(b, self, sizeof *self / 4);
    umbra_ptr const colors = umbra_bind_sealed(b, &self->colors);
    umbra_ptr const pos    = umbra_bind_sealed(b, &self->pos);
