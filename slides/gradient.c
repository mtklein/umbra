...
static void grad_2stop_render_row(
        slide *s, int y, int w,
    ...
    slide_uni_f32(uni, lay->shader + 12,
        s->color, 8);
    for (int i = 0; i < ps; i++) {
        slide_uni_i32(uni,
...
slide slide_gradient_2stop(char const*, uint32_t,
                          umbra_shader_fn,
                          umbra_store_fn,
                          float const[8],
                          float const[4]);
slide slide_gradient_lut(char const*, uint32_t,
                         umbra_shader_fn,
                         umbra_store_fn,
                         float const[4],
                         float*, int);

slide slide_gradient_2stop(
        char const *title, uint32_t bg,
