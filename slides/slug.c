...
struct slug_polyline slug_polyline_extract(char const *text, float font_size,
    ...
    struct slug_polyline sp = {0};
    if (samples_per_curve < 1) { samples_per_curve = 1; }
    ...
    for (int i = 0; text[i]; i++) {
        ...
        int nv = stbtt_GetCodepointShape(&font, text[i], &v);
        for (int j = 0; j < nv; j++) {
            if      (v[j].type == STBTT_vline)  { total += 1; }
            else if (v[j].type == STBTT_vcurve) { total += samples_per_curve; }
        }
        stbtt_FreeShape(&font, v);
    }

    sp.data = malloc((size_t)total * 4 * sizeof(float));
    float baseline = (float)asc * scale;
    float cx = 0;
        ...
        for (int j = 0; j < nv; j++) {
            ...
            if (v[j].type == STBTT_vmove) {
                ...
                for (int s = 0; s < samples_per_curve; s++) {
                    float const t  = (float)(s + 1) / (float)samples_per_curve,
                                mt = 1.0f - t;
                    float const bx = mt*mt*px + 2.0f*mt*t*kx + t*t*vx,
                                by = mt*mt*py + 2.0f*mt*t*ky + t*t*vy;
                    float *d = sp.data + sp.count * 4;
                    d[0] = ax; d[1] = ay;
                    d[2] = bx; d[3] = by;
                    sp.count++;
    ...
    for (int i = 0; i < sp.count; i++) {
        float *d = sp.data + i * 4;
        for (int k = 0; k < 2; k++) {
            if (d[2*k]   < lx) { lx = d[2*k];   } if (d[2*k]   > hx) { hx = d[2*k];   }
            if (d[2*k+1] < ly) { ly = d[2*k+1]; } if (d[2*k+1] > hy) { hy = d[2*k+1]; }
        }
    }
    lx -= 1.0f; ly -= 1.0f;
    hx += 1.0f; hy += 1.0f;
