...
static void dump(const char *name,
    ...
    { char p[64]; snprintf(p,sizeof p,"dumps/%s.opt",name);
      FILE *f = fopen(p,"w"); umbra_basic_block_dump(bb,f); fclose(f); }
    ...
    if (cg) { char p[64]; snprintf(p,sizeof p,"dumps/%s.c",name);
              FILE *f = fopen(p,"w"); umbra_codegen_dump(cg,f); fclose(f); }
#ifdef JIT_EXT
    if (jit) { char p[64]; snprintf(p,sizeof p,"dumps/%s." JIT_EXT,name);
               FILE *f = fopen(p,"w"); umbra_jit_dump(jit,f); fclose(f); }
    if (jit) { char p[64]; snprintf(p,sizeof p,"dumps/%s." JIT_EXT ".mca",name);
               FILE *f = fopen(p,"w"); umbra_jit_mca(jit,f); fclose(f); }
#endif
    if (mtl) { char p[64]; snprintf(p,sizeof p,"dumps/%s.metal",name);
               FILE *f = fopen(p,"w"); umbra_metal_dump(mtl,f); fclose(f); }
...
int main(void) {
    ...
    dump("solid_src_8888",     umbra_shader_solid, NULL, umbra_blend_src,     umbra_load_8888, umbra_store_8888);
    dump("solid_srcover_8888", umbra_shader_solid, NULL, umbra_blend_srcover, umbra_load_8888, umbra_store_8888);
