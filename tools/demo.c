...
static void fill_bg_row(void *dst, int n, uint32_t bg, size_t row_sz, size_t plane_gap) {
    ...
    umbra_buf buf[5];
    buf[0] = (umbra_buf){uni, (size_t)fill_pipe.uni_len, 1, 0};
...
static void readback_row(uint32_t *dst, void *src, int n, size_t src_sz, size_t plane_gap) {
    ...
    umbra_buf buf[6];
    buf[0] = (umbra_buf){uni, (size_t)readback_pipe.uni_len, 1, 0};
...
static void to_hdr_row(__fp16 *dst, void *src, int n, size_t src_sz, size_t plane_gap) {
    ...
    umbra_buf buf[6];
    buf[0] = (umbra_buf){uni, (size_t)hdr_pipe.uni_len, 1, 0};
    buf[1] = (umbra_buf){src, src_sz, 1, 0};
    for (int i = 0; i < ps; i++) {
        buf[2 + i] = (umbra_buf){(char *)src + (size_t)(i + 1) * plane_gap, src_sz, 0, 0};
    }
    buf[op] = (umbra_buf){dst, (size_t)(n * 8), 0, 0};
