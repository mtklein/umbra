...
struct ra* ra_create(struct umbra_basic_block const *bb,
                     struct ra_config const *cfg) {
    int n = bb->insts;
    struct ra *ra = malloc(sizeof *ra);
    ra->cfg      = *cfg;
    ra->reg      = malloc((size_t)n * sizeof *ra->reg);
    ra->reg_hi   = malloc((size_t)n
                          * sizeof *ra->reg_hi);
    ra->last_use = malloc((size_t)n
                          * sizeof *ra->last_use);
    ra->is_pair  = malloc((size_t)n
                          * sizeof *ra->is_pair);
    ra->owner    = malloc((size_t)cfg->max_reg
                          * sizeof *ra->owner);
    ra->free_stack = malloc((size_t)cfg->max_reg
                            * sizeof *ra->free_stack);
    ...
    ra->loop_reg = malloc((size_t)bb->preamble
                          * sizeof *ra->loop_reg);
...
int8_t ra_alloc(struct ra *ra, int *sl, int *ns) {
    ...
    // Evict: farthest last_use (Belady-like).
    ...
    ra->cfg.spill(ra->reg[evicted], sl[evicted],
                  ra->cfg.ctx);
    ra->owner[(int)ra->reg[evicted]] = -1;
    if (ra->reg_hi[evicted] >= 0) {
        ra->cfg.spill(ra->reg_hi[evicted],
                      sl[evicted]+1, ra->cfg.ctx);
...
int8_t ra_ensure(struct ra *ra, int *sl, int *ns,
                 int val) {
...
int8_t ra_claim(struct ra *ra, int old_val,
                int new_val) {
...
static struct ra_step step0(void) {
    return (struct ra_step){
        ...
        .rx=-1, .ry=-1, .rz=-1,
        .rxh=-1, .ryh=-1, .rzh=-1,
        .scratch=-1, .scratch2=-1,
    };
}

struct ra_step ra_step_alloc(struct ra *ra,
                             int *sl, int *ns,
                             int i) {
    ...
    if (ra->is_pair[i]) {
        ...
        ra->reg_hi[i] = s.rdh;
        ra->owner[(int)s.rdh] = i;
    }
    return s;
}

struct ra_step ra_step_unary(struct ra *ra,
                             int *sl, int *ns,
    ...
    if (out_pair && !in_pair) {
        // Widening: can't claim (need 2 regs from 1).
        s.rd  = ra_alloc(ra, sl, ns);
        s.rdh = ra_alloc(ra, sl, ns);
        ra->reg[i]    = s.rd;
        ra->reg_hi[i] = s.rdh;
        ...
        s.rdh = ra->reg_hi[i];
        // Narrowing: free the unneeded hi register.
...
struct ra_step ra_step_alu(struct ra *ra,
                           int *sl, int *ns,
                           struct bb_inst const *inst,
                           int i, _Bool scalar,
                           int nscratch) {
    ...
    enum op op = inst->op;
    _Bool fma = op == op_fma_f32
             || op == op_fms_f32;
    _Bool destructive = fma || op == op_sel_32;

    _Bool can_claim_x =
        !pair || ra->is_pair[inst->x];
    _Bool can_claim_y =
        !pair || ra->is_pair[inst->y];
    _Bool can_claim_z =
        !pair || ra->is_pair[inst->z];
    ...
    if (!destructive) {
        if (s.rd < 0 && x_dead && can_claim_x) {
            s.rd = ra_claim(ra, inst->x, i);
            x_dead = 0;
        }
        if (s.rd < 0 && y_dead && can_claim_y) {
            s.rd = ra_claim(ra, inst->y, i);
            y_dead = 0;
        }
        if (s.rd < 0 && z_dead && can_claim_z) {
            s.rd = ra_claim(ra, inst->z, i);
            z_dead = 0;
        }
    }
