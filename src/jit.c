    ...
    for (int i = from; i < to; i++) {
        ...
        case op_scatter_16: {
            ...
            if (scalar) {
                ...
                int p = inst->ptr;
                int base = resolve_ptr_x86(c, p,
                    &last_ptr, deref_gpr);
                sub_ri(c, RSP, 80);
                vmov_store(c,0,ry,RSP,RSP,0,0);
                vmov_store(c,1,rx,RSP,RSP,0,16);
                vmov_store(c,1,rz,RSP,RSP,0,48);
                if (lu(inst->x) <= i) {
                    ra_free_reg(ra, inst->x);
                }
                if (base != R11) {
                ...
                for (int k = 0; k < 8; k++) {
                    ...
                    emit1(c, 0x44);
                    emit1(c, 0x24);
                    emit1(c,(uint8_t)(48+k*4));
                    test_rr(c, RAX, RAX);
                    int skip = jcc(c, 0x04);
                    emit1(c, 0x8b);
                    emit1(c, 0x44);
                    emit1(c, 0x24);
                    emit1(c,(uint8_t)(16+k*4));
                    emit1(c, 0x48);
                    emit1(c, 0x63);
                    emit1(c, 0xc0);
                    emit1(c, 0x0f);
