...
typedef umbra_i16       i16;
typedef umbra_i32       i32;
typedef umbra_f32       f32;
typedef umbra_half      vh;
typedef umbra_half_mask vm;
...
i32 umbra_lane(BB *bb) {
    return (i32){push(bb, op_lane)};
}

i16 umbra_imm_i16      (BB *bb, uint16_t bits) { return (i16){push(bb, op_imm_16  , .imm=(int)bits)}; }
i32 umbra_imm_i32      (BB *bb, uint32_t bits) { return (i32){push(bb, op_imm_32  , .imm=(int)bits)}; }
f32 umbra_imm_f32      (BB *bb, uint32_t bits) { return (f32){push(bb, op_imm_32  , .imm=(int)bits)}; }
...
static int math_(BB *bb, struct bb_inst inst) {
    if (is_imm(bb, inst.x) && is_imm(bb, inst.y) && is_imm(bb, inst.z)) {
        ...
        return output_type(inst.op) == OP_HALF ? umbra_imm_half(bb, (uint16_t)result).id
             : output_type(inst.op) == OP_16   ? umbra_imm_i16 (bb, (uint16_t)result).id
             :                                   umbra_imm_i32 (bb, (uint32_t)result).id;
    }
...
f32 umbra_add_f32(BB *bb, f32 a, f32 b) {
    sort(&a.id, &b.id);
    int const x = a.id,
              y = b.id;
    if (bb->inst[x].op == op_mul_f32) {
        return (f32){math(bb, op_fma_f32, .x=bb->inst[x].x, .y=bb->inst[x].y, .z=y)};
    }
    if (bb->inst[y].op == op_mul_f32) {
        return (f32){math(bb, op_fma_f32, .x=bb->inst[y].x, .y=bb->inst[y].y, .z=x)};
    }
    return (f32){math(bb, op_add_f32, .x=x, .y=y)};
}

f32 umbra_sub_f32(BB *bb, f32 a, f32 b) {
    if (is_imm32(bb, b.id, 0)) { return a; }
    if (bb->inst[b.id].op == op_mul_f32) {
        return (f32){math(bb, op_fms_f32, .x=bb->inst[b.id].x, .y=bb->inst[b.id].y, .z=a.id)};
    }
    return (f32){math(bb, op_sub_f32, .x=a.id, .y=b.id)};
}

f32 umbra_mul_f32(BB *bb, f32 a, f32 b) {
    sort(&a.id, &b.id);
    if (is_imm32(bb, a.id, 0x3f800000)) { return b; }
    if (is_imm32(bb, b.id, 0x3f800000)) { return a; }
    return (f32){math(bb, op_mul_f32, .x=a.id, .y=b.id)};
}

f32 umbra_div_f32(BB *bb, f32 a, f32 b) {
    if (is_imm32(bb, b.id, 0x3f800000)) { return a; }
    return (f32){math(bb, op_div_f32, .x=a.id, .y=b.id)};
}

f32 umbra_min_f32(BB *bb, f32 a, f32 b) {
    sort(&a.id, &b.id);
    return (f32){math(bb, op_min_f32, .x=a.id, .y=b.id)};
}

f32 umbra_max_f32(BB *bb, f32 a, f32 b) {
    sort(&a.id, &b.id);
    return (f32){math(bb, op_max_f32, .x=a.id, .y=b.id)};
}

f32 umbra_sqrt_f32(BB *bb, f32 a) {
    return (f32){math(bb, op_sqrt_f32, .x=a.id)};
}
...
i32 umbra_add_i32(BB *bb, i32 a, i32 b) {
    sort(&a.id, &b.id);
    if (is_imm32(bb, a.id, 0)) { return b; }
    if (is_imm32(bb, b.id, 0)) { return a; }
    return (i32){math(bb, op_add_i32, .x=a.id, .y=b.id)};
}

i32 umbra_sub_i32(BB *bb, i32 a, i32 b) {
    if (is_imm32(bb, b.id, 0)) { return a; }
    if (a.id == b.id) { return umbra_imm_i32(bb, 0); }
    return (i32){math(bb, op_sub_i32, .x=a.id, .y=b.id)};
}

i32 umbra_mul_i32(BB *bb, i32 a, i32 b) {
    sort(&a.id, &b.id);
    ...
    if (bb->inst[a.id].op == op_imm_32 && is_pow2(bb->inst[a.id].imm)) {
        ...
        return umbra_shl_i32(bb, b, umbra_imm_i32(bb, (uint32_t)shift));
    }
    if (bb->inst[b.id].op == op_imm_32 && is_pow2(bb->inst[b.id].imm)) {
        int const shift = __builtin_ctz((unsigned)bb->inst[b.id].imm);
        return umbra_shl_i32(bb, a, umbra_imm_i32(bb, (uint32_t)shift));
    }
    return (i32){math(bb, op_mul_i32, .x=a.id, .y=b.id)};
}

i16 umbra_add_i16(BB *bb, i16 a, i16 b) {
    sort(&a.id, &b.id);
    if (is_imm16(bb, a.id, 0)) { return b; }
    if (is_imm16(bb, b.id, 0)) { return a; }
    return (i16){math(bb, op_add_i16, .x=a.id, .y=b.id)};
}

i16 umbra_sub_i16(BB *bb, i16 a, i16 b) {
    if (is_imm16(bb, b.id, 0)) { return a; }
    if (a.id == b.id) { return umbra_imm_i16(bb, 0); }
    return (i16){math(bb, op_sub_i16, .x=a.id, .y=b.id)};
}


i16 umbra_mul_i16(BB *bb, i16 a, i16 b) {
    sort(&a.id, &b.id);
    ...
    if (bb->inst[a.id].op == op_imm_16 && is_pow2(bb->inst[a.id].imm)) {
        int const shift = __builtin_ctz((unsigned)bb->inst[a.id].imm);
        return umbra_shl_i16(bb, b, umbra_imm_i16(bb, (uint16_t)shift));
    }
    if (bb->inst[b.id].op == op_imm_16 && is_pow2(bb->inst[b.id].imm)) {
        int const shift = __builtin_ctz((unsigned)bb->inst[b.id].imm);
        return umbra_shl_i16(bb, a, umbra_imm_i16(bb, (uint16_t)shift));
    }
    return (i16){math(bb, op_mul_i16, .x=a.id, .y=b.id)};
}

i32 umbra_shl_i32(BB *bb, i32 a, i32 b) {
    if (is_imm32(bb, b.id, 0)) { return a; }
    if (is_imm(bb, b.id)) { return (i32){push(bb, op_shl_i32_imm, .x=a.id, .imm=bb->inst[b.id].imm)}; }
    return (i32){math(bb, op_shl_i32, .x=a.id, .y=b.id)};
}
i32 umbra_shr_u32(BB *bb, i32 a, i32 b) {
    if (is_imm32(bb, b.id, 0)) { return a; }
    if (is_imm(bb, b.id)) { return (i32){push(bb, op_shr_u32_imm, .x=a.id, .imm=bb->inst[b.id].imm)}; }
    return (i32){math(bb, op_shr_u32, .x=a.id, .y=b.id)};
}
i32 umbra_shr_s32(BB *bb, i32 a, i32 b) {
    if (is_imm32(bb, b.id, 0)) { return a; }
    if (is_imm(bb, b.id)) { return (i32){push(bb, op_shr_s32_imm, .x=a.id, .imm=bb->inst[b.id].imm)}; }
    return (i32){math(bb, op_shr_s32, .x=a.id, .y=b.id)};
}

i16 umbra_shl_i16(BB *bb, i16 a, i16 b) {
    if (is_imm16(bb, b.id, 0)) { return a; }
    if (is_imm(bb, b.id)) { return (i16){push(bb, op_shl_i16_imm, .x=a.id, .imm=bb->inst[b.id].imm)}; }
    return (i16){math(bb, op_shl_i16, .x=a.id, .y=b.id)};
}
i16 umbra_shr_u16(BB *bb, i16 a, i16 b) {
    if (is_imm16(bb, b.id, 0)) { return a; }
    if (is_imm(bb, b.id)) { return (i16){push(bb, op_shr_u16_imm, .x=a.id, .imm=bb->inst[b.id].imm)}; }
    return (i16){math(bb, op_shr_u16, .x=a.id, .y=b.id)};
}
i16 umbra_shr_s16(BB *bb, i16 a, i16 b) {
    if (is_imm16(bb, b.id, 0)) { return a; }
    if (is_imm(bb, b.id)) { return (i16){push(bb, op_shr_s16_imm, .x=a.id, .imm=bb->inst[b.id].imm)}; }
    return (i16){math(bb, op_shr_s16, .x=a.id, .y=b.id)};
}

i32 umbra_and_32(BB *bb, i32 a, i32 b) {
    sort(&a.id, &b.id);
    ...
    if (is_imm32(bb, a.id,  0)) { return a; }
    if (is_imm32(bb, b.id,  0)) { return b; }
    return (i32){math(bb, op_and_32, .x=a.id, .y=b.id)};
}
i32 umbra_or_32(BB *bb, i32 a, i32 b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return a; }
    if (is_imm32(bb, a.id,  0)) { return b; }
    if (is_imm32(bb, b.id,  0)) { return a; }
    ...
    return (i32){math(bb, op_or_32, .x=a.id, .y=b.id)};
}
i32 umbra_xor_32(BB *bb, i32 a, i32 b) {
    sort(&a.id, &b.id);
    if (is_imm32(bb, a.id, 0)) { return b; }
    if (is_imm32(bb, b.id, 0)) { return a; }
    if (a.id == b.id) { return umbra_imm_i32(bb, 0); }
    return (i32){math(bb, op_xor_32, .x=a.id, .y=b.id)};
}
i32 umbra_sel_i32(BB *bb, i32 c, i32 t, i32 fv) {
    if (t.id == fv.id) { return t; }
    if (is_imm32(bb, c.id, -1)) { return t; }
    if (is_imm32(bb, c.id,  0)) { return fv; }
    return (i32){math(bb, op_sel_32, .x=c.id, .y=t.id, .z=fv.id)};
}
f32 umbra_sel_f32(BB *bb, i32 c, f32 t, f32 fv) {
    if (t.id == fv.id) { return t; }
    if (is_imm32(bb, c.id, -1)) { return t; }
    if (is_imm32(bb, c.id,  0)) { return fv; }
    return (f32){math(bb, op_sel_32, .x=c.id, .y=t.id, .z=fv.id)};
}

i16 umbra_and_16(BB *bb, i16 a, i16 b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return a; }
    ...
    if (is_imm16(bb, a.id,      0)) { return a; }
    if (is_imm16(bb, b.id,      0)) { return b; }
    return (i16){math(bb, op_and_16, .x=a.id, .y=b.id)};
}
i16 umbra_or_16(BB *bb, i16 a, i16 b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return a; }
    if (is_imm16(bb, a.id,      0)) { return b; }
    if (is_imm16(bb, b.id,      0)) { return a; }
    ...
    return (i16){math(bb, op_or_16, .x=a.id, .y=b.id)};
}
i16 umbra_xor_16(BB *bb, i16 a, i16 b) {
    sort(&a.id, &b.id);
    if (is_imm16(bb, a.id, 0)) { return b; }
    if (is_imm16(bb, b.id, 0)) { return a; }
    if (a.id == b.id) { return umbra_imm_i16(bb, 0); }
    return (i16){math(bb, op_xor_16, .x=a.id, .y=b.id)};
}
i16 umbra_sel_16(BB *bb, i16 c, i16 t, i16 fv) {
    if (t.id == fv.id) { return t; }
    if (is_imm16(bb, c.id, 0xffff)) { return t; }
    if (is_imm16(bb, c.id,      0)) { return fv; }
    return (i16){math(bb, op_sel_16, .x=c.id, .y=t.id, .z=fv.id)};
}

vm umbra_and_half(BB *bb, vm a, vm b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return a; }
    ...
    return (vm){math(bb, op_and_half, .x=a.id, .y=b.id)};
}
vm umbra_or_half(BB *bb, vm a, vm b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return a; }
    ...
    return (vm){math(bb, op_or_half, .x=a.id, .y=b.id)};
}
vm umbra_xor_half(BB *bb, vm a, vm b) {
    sort(&a.id, &b.id);
    if (is_imm_half(bb, a.id, 0)) { return b; }
    if (is_imm_half(bb, b.id, 0)) { return a; }
    if (a.id == b.id) { return umbra_imm_half_mask(bb, 0); }
    return (vm){math(bb, op_xor_half, .x=a.id, .y=b.id)};
}
vh umbra_sel_half(BB *bb, vm c, vh t, vh fv) {
    if (t.id == fv.id) { return t; }
    if (is_imm_half(bb, c.id, 0xffff)) { return t; }
    if (is_imm_half(bb, c.id,      0)) { return fv; }
    return (vh){math(bb, op_sel_half, .x=c.id, .y=t.id, .z=fv.id)};
}

vh umbra_half_from_f32(BB *bb, f32 a) {
    return (vh){math(bb, op_half_from_f32, .x=a.id)};
}
f32 umbra_f32_from_half(BB *bb, vh a) {
    return (f32){math(bb, op_f32_from_half, .x=a.id)};
}
f32 umbra_f32_from_i32(BB *bb, i32 a) {
    return (f32){math(bb, op_f32_from_i32, .x=a.id)};
}
i32 umbra_i32_from_f32(BB *bb, f32 a) {
    return (i32){math(bb, op_i32_from_f32, .x=a.id)};
}
vh umbra_half_from_i32(BB *bb, i32 a) {
    return (vh){math(bb, op_half_from_i32, .x=a.id)};
}
vh umbra_half_from_i16(BB *bb, i16 a) {
    return (vh){math(bb, op_half_from_i16, .x=a.id)};
}
i32 umbra_i32_from_half(BB *bb, vh a) {
    return (i32){math(bb, op_i32_from_half, .x=a.id)};
}
i16 umbra_i16_from_half(BB *bb, vh a) {
    return (i16){math(bb, op_i16_from_half, .x=a.id)};
}
i16 umbra_i16_from_i32(BB *bb, i32 a) {
    if (bb->inst[a.id].op == op_i32_from_i16) { return (i16){bb->inst[a.id].x}; }
    if (bb->inst[a.id].op == op_shr_u32_imm) {
        return (i16){push(bb, op_shr_narrow_u32, .x=bb->inst[a.id].x, .imm=bb->inst[a.id].imm)};
    }
    return (i16){math(bb, op_i16_from_i32, .x=a.id)};
}
i32 umbra_i32_from_i16(BB *bb, i16 a) {
    return (i32){math(bb, op_i32_from_i16, .x=a.id)};
}
vm umbra_half_mask_from_i32(BB *bb, i32 a) {
    return (vm){math(bb, op_i16_from_i32, .x=a.id)};
}

void umbra_load_8x4(BB *bb, umbra_ptr src, i32 ix, i16 out[4]) {
    (void)ix;
    int base = push(bb, op_load_8x4, .ptr=src.ix);
    for (int ch = 1; ch <= 3; ch++) {
        push(bb, op_load_8x4, .x=base, .imm=ch);
    }
    for (int ch = 0; ch < 4; ch++) { out[ch] = (i16){base + ch}; }
}

void umbra_store_8x4(BB *bb, umbra_ptr dst, i32 ix, i16 in[4]) {
    (void)ix;
    push(bb, op_store_8x4, .x=in[0].id, .y=in[1].id, .z=in[2].id, .w=in[3].id, .ptr=dst.ix);
}


vm umbra_eq_half(BB *bb, vh a, vh b) {
    sort(&a.id, &b.id);
    return (vm){math(bb, op_eq_half, .x=a.id, .y=b.id)};
}
vm umbra_ne_half(BB *bb, vh a, vh b) {
    return umbra_xor_half(bb, umbra_eq_half(bb, a, b), umbra_imm_half_mask(bb, 0xffff));
}
vm umbra_lt_half(BB *bb, vh a, vh b) { return (vm){math(bb, op_lt_half, .x=a.id, .y=b.id)}; }
vm umbra_le_half(BB *bb, vh a, vh b) { return (vm){math(bb, op_le_half, .x=a.id, .y=b.id)}; }
vm umbra_gt_half(BB *bb, vh a, vh b) { return umbra_lt_half(bb, b, a); }
vm umbra_ge_half(BB *bb, vh a, vh b) { return umbra_le_half(bb, b, a); }

i32 umbra_eq_f32(BB *bb, f32 a, f32 b) {
    sort(&a.id, &b.id);
    return (i32){math(bb, op_eq_f32, .x=a.id, .y=b.id)};
}
i32 umbra_ne_f32(BB *bb, f32 a, f32 b) {
    return umbra_xor_32(bb, umbra_eq_f32(bb, a, b), umbra_imm_i32(bb, 0xffffffff));
}
i32 umbra_lt_f32(BB *bb, f32 a, f32 b) { return (i32){math(bb, op_lt_f32, .x=a.id, .y=b.id)}; }
i32 umbra_le_f32(BB *bb, f32 a, f32 b) { return (i32){math(bb, op_le_f32, .x=a.id, .y=b.id)}; }
i32 umbra_gt_f32(BB *bb, f32 a, f32 b) { return umbra_lt_f32(bb, b, a); }
i32 umbra_ge_f32(BB *bb, f32 a, f32 b) { return umbra_le_f32(bb, b, a); }

i16 umbra_eq_i16(BB *bb, i16 a, i16 b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return umbra_imm_i16(bb, 0xffff); }
    return (i16){math(bb, op_eq_i16, .x=a.id, .y=b.id)};
}
i16 umbra_ne_i16(BB *bb, i16 a, i16 b) {
    return umbra_xor_16(bb, umbra_eq_i16(bb, a, b), umbra_imm_i16(bb, 0xffff));
}
i32 umbra_eq_i32(BB *bb, i32 a, i32 b) {
    sort(&a.id, &b.id);
    if (a.id == b.id) { return umbra_imm_i32(bb, 0xffffffff); }
    return (i32){math(bb, op_eq_i32, .x=a.id, .y=b.id)};
}
i32 umbra_ne_i32(BB *bb, i32 a, i32 b) {
    return umbra_xor_32(bb, umbra_eq_i32(bb, a, b), umbra_imm_i32(bb, 0xffffffff));
}

i16 umbra_lt_s16(BB *bb, i16 a, i16 b) { return (i16){math(bb, op_lt_s16, .x=a.id, .y=b.id)}; }
i16 umbra_le_s16(BB *bb, i16 a, i16 b) { return (i16){math(bb, op_le_s16, .x=a.id, .y=b.id)}; }
i16 umbra_gt_s16(BB *bb, i16 a, i16 b) { return umbra_lt_s16(bb, b, a); }
i16 umbra_ge_s16(BB *bb, i16 a, i16 b) { return umbra_le_s16(bb, b, a); }
i32 umbra_lt_s32(BB *bb, i32 a, i32 b) { return (i32){math(bb, op_lt_s32, .x=a.id, .y=b.id)}; }
i32 umbra_le_s32(BB *bb, i32 a, i32 b) { return (i32){math(bb, op_le_s32, .x=a.id, .y=b.id)}; }
i32 umbra_gt_s32(BB *bb, i32 a, i32 b) { return umbra_lt_s32(bb, b, a); }
i32 umbra_ge_s32(BB *bb, i32 a, i32 b) { return umbra_le_s32(bb, b, a); }

i16 umbra_lt_u16(BB *bb, i16 a, i16 b) { return (i16){math(bb, op_lt_u16, .x=a.id, .y=b.id)}; }
i16 umbra_le_u16(BB *bb, i16 a, i16 b) { return (i16){math(bb, op_le_u16, .x=a.id, .y=b.id)}; }
i16 umbra_gt_u16(BB *bb, i16 a, i16 b) { return umbra_lt_u16(bb, b, a); }
i16 umbra_ge_u16(BB *bb, i16 a, i16 b) { return umbra_le_u16(bb, b, a); }
i32 umbra_lt_u32(BB *bb, i32 a, i32 b) { return (i32){math(bb, op_lt_u32, .x=a.id, .y=b.id)}; }
i32 umbra_le_u32(BB *bb, i32 a, i32 b) { return (i32){math(bb, op_le_u32, .x=a.id, .y=b.id)}; }
i32 umbra_gt_u32(BB *bb, i32 a, i32 b) { return umbra_lt_u32(bb, b, a); }
i32 umbra_ge_u32(BB *bb, i32 a, i32 b) { return umbra_le_u32(bb, b, a); }
