...
val umbra_widen_u16(builder *b, val x) {
    return push(b, op_widen_u16, .x=x.id);
}
val umbra_narrow_i16(builder *b, val x) {
    return push(b, op_narrow_16, .x=x.id);
}

val umbra_widen_f16(builder *b, val x) {
    return push(b, op_widen_f16, .x=x.id);
}
val umbra_narrow_f32(builder *b, val x) {
    return push(b, op_narrow_f32, .x=x.id);
}
...
static val try_imm(builder *b, val d,
                   enum op fused,
                   int x, int y) {
    int imm_id = is_imm(b, x) ? x
               : is_imm(b, y) ? y : -1;
    if (imm_id >= 0) {
...
val umbra_add_f32(builder *b, val x, val y) {
    ...
    return try_imm(b,
        math(b, op_add_f32, .x=x.id, .y=y.id),
        op_add_f32_imm, x.id, y.id);
}
...
val umbra_sub_f32(builder *b, val x, val y) {
    ...
    val d = math(b, op_sub_f32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_sub_f32_imm,
                     .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
...
val umbra_mul_f32(builder *b, val x, val y) {
    ...
    return try_imm(b,
        math(b, op_mul_f32, .x=x.id, .y=y.id),
        op_mul_f32_imm, x.id, y.id);
}

val umbra_div_f32(builder *b, val x, val y) {
    if (is_imm32(b, y.id, 0x3f800000)) { return x; }
    val d = math(b, op_div_f32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_div_f32_imm,
                     .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
...
val umbra_min_f32(builder *b, val x, val y) {
    sort(&x.id, &y.id);
    return try_imm(b,
        math(b, op_min_f32, .x=x.id, .y=y.id),
        op_min_f32_imm, x.id, y.id);
}
...
val umbra_sub_i32(builder *b, val x, val y) {
    ...
    val d = math(b, op_sub_i32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_sub_i32_imm,
                     .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
...
val umbra_mul_i32(builder *b, val x, val y) {
    ...
    return try_imm(b,
        math(b, op_mul_i32, .x=x.id, .y=y.id),
        op_mul_i32_imm, x.id, y.id);
}

val umbra_shl_i32(builder *b, val x, val y) {
    if (is_imm32(b, y.id, 0)) { return x; }
    if (is_imm(b, y.id)) {
        return push(b, op_shl_imm,
                         .x=x.id,
                         .imm=b->inst[y.id].imm);
    }
    return math(b, op_shl_i32,
                      .x=x.id, .y=y.id);
}
val umbra_shr_u32(builder *b, val x, val y) {
    if (is_imm32(b, y.id, 0)) { return x; }
    if (is_imm(b, y.id)) {
        return push(b, op_shr_u32_imm,
                         .x=x.id,
                         .imm=b->inst[y.id].imm);
    }
    return math(b, op_shr_u32,
                      .x=x.id, .y=y.id);
}
val umbra_shr_s32(builder *b, val x, val y) {
    if (is_imm32(b, y.id, 0)) { return x; }
    if (is_imm(b, y.id)) {
        return push(b, op_shr_s32_imm,
                         .x=x.id,
                         .imm=b->inst[y.id].imm);
    }
    return math(b, op_shr_s32,
                      .x=x.id, .y=y.id);
}
...
val umbra_cvt_f32_i32(builder *b, val x) {
    return math(b, op_f32_from_i32, .x=x.id);
}
val umbra_cvt_i32_f32(builder *b, val x) {
    return math(b, op_i32_from_f32, .x=x.id);
}

val umbra_eq_f32(builder *b, val x, val y) {
    sort(&x.id, &y.id);
    return try_imm(b,
        math(b, op_eq_f32, .x=x.id, .y=y.id),
        op_eq_f32_imm, x.id, y.id);
}
...
val umbra_lt_f32(builder *b, val x, val y) {
    val d = math(b, op_lt_f32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_lt_f32_imm, .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
    }
...
val umbra_le_f32(builder *b, val x, val y) {
    val d = math(b, op_le_f32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_le_f32_imm, .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
    }
...
val umbra_eq_i32(builder *b, val x, val y) {
    ...
    return try_imm(b,
        math(b, op_eq_i32, .x=x.id, .y=y.id),
        op_eq_i32_imm, x.id, y.id);
}
...
val umbra_lt_s32(builder *b, val x, val y) {
    val d = math(b, op_lt_s32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_lt_s32_imm, .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
    }
...
val umbra_le_s32(builder *b, val x, val y) {
    val d = math(b, op_le_s32,
                       .x=x.id, .y=y.id);
    if (is_imm(b, y.id)) {
        val f = push(b, op_le_s32_imm, .x=x.id,
                     .imm=b->inst[y.id].imm);
        return push(b, op_join,
                    .x=d.id, .y=f.id);
    }
...
val umbra_lt_u32(builder *b, val x, val y) {
    return math(b, op_lt_u32,
                      .x=x.id, .y=y.id);
}
val umbra_le_u32(builder *b, val x, val y) {
    return math(b, op_le_u32,
                      .x=x.id, .y=y.id);
}
