...
struct umbra_metal* umbra_metal(void *ctx,
        struct umbra_basic_block const *bb) {
    (void)ctx; (void)bb; return 0;
}
void umbra_metal_run(struct umbra_metal *m,
                     int n, umbra_buf buf[]) {
...
void umbra_dump_metal(struct umbra_metal const *m,
                      FILE *f) { (void)m; (void)f; }
