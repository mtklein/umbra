...
void vzeroupper(Buf *b) { emit1(b,0xc5); emit1(b,0xf8); emit1(b,0x77); }
...
void vpslld_i(Buf *b, int d, int s, uint8_t imm) { vex_shift(b,1,1,1,0x72,6,d,s,imm); }
void vpsrld_i(Buf *b, int d, int s, uint8_t imm) { vex_shift(b,1,1,1,0x72,2,d,s,imm); }
void vpsrad_i(Buf *b, int d, int s, uint8_t imm) { vex_shift(b,1,1,1,0x72,4,d,s,imm); }
...
void vpmaskmov_load(Buf *b, int L, int dst, int mask,
                    int base, int index, int scale, int disp) {
    vex_mem(b, 1, 2, 0, L, dst, mask, 0x8c, base, index, scale, disp);
}
void vpmaskmov_store(Buf *b, int L, int mask, int src,
                     int base, int index, int scale, int disp) {
    vex_mem(b, 1, 2, 0, L, src, mask, 0x8e, base, index, scale, disp);
}
