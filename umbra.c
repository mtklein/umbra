...
static _Bool imm32_is(struct umbra_inst const inst[], int j, int val) {
    return inst[j].op == umbra_imm_32 && inst[j].immi == val;
}
static _Bool imm16_is(struct umbra_inst const inst[], int j, int val) {
    return inst[j].op == umbra_imm_16 && (int16_t)inst[j].immi == (int16_t)val;
}
        ...
        for (int i = 0; i < insts; i++) {
            ...
            switch (inst[i].op) {
                ...
                case umbra_mul_i32:
                    if      (imm32_is(inst, y, 1)) { inst[i] = inst[x]; }
                        ...
                        if (inst[x].op == umbra_imm_32 && inst[x].immi > 0
                            && (inst[x].immi & (inst[x].immi - 1)) == 0 && uses[x] == 1) {
                            imm_idx = x; val_idx = y;
                        } else if (inst[y].op == umbra_imm_32 && inst[y].immi > 0
                ...
                case umbra_mul_i16:
                    if      (imm16_is(inst, y, 1)) { inst[i] = inst[x]; }
                        ...
                        if (inst[x].op == umbra_imm_16 && px > 0
                            && (px & (px - 1)) == 0 && uses[x] == 1) {
                            imm_idx = x; val_idx = y;
                        } else if (inst[y].op == umbra_imm_16 && py > 0
                            && (py & (py - 1)) == 0 && uses[y] == 1) {
                            imm_idx = y; val_idx = x;
                        }
                        if (imm_idx >= 0) {
                ...
                case umbra_sub_i32:
                    if      (imm32_is(inst, y, 0)) { inst[i] = inst[x]; }
                    else if (x == y) { inst[i].op = umbra_imm_32; inst[i].immi = 0;
                                       inst[i].x = inst[i].y = inst[i].z = 0; }
                    else { reduced = 0; }
                    break;
                case umbra_sub_i16:
                    if      (imm16_is(inst, y, 0)) { inst[i] = inst[x]; }
                    else if (x == y) { inst[i].op = umbra_imm_16; inst[i].immi = 0;
                                       inst[i].x = inst[i].y = inst[i].z = 0; }
                    else { reduced = 0; }
                    break;
                ...
                case umbra_shl_i32: case umbra_shr_u32: case umbra_shr_s32:
                    if (imm32_is(inst, y, 0)) { inst[i] = inst[x]; }
                    else { reduced = 0; }
                    break;
                case umbra_shl_i16: case umbra_shr_u16: case umbra_shr_s16:
                ...
                case umbra_fma_f32:
                    if (imm32_is(inst, z, 0)) {
                        inst[i].op = umbra_mul_f32; inst[i].z = 0;
                    } else if (imm32_is(inst, x, f32_bits(1.0f))) {
                        inst[i].op = umbra_add_f32;
                        inst[i].x = y; inst[i].y = z; inst[i].z = 0;
                    } else if (imm32_is(inst, y, f32_bits(1.0f))) {
                        inst[i].op = umbra_add_f32; inst[i].y = z; inst[i].z = 0;
                    } else { reduced = 0; }
                    break;
                case umbra_fma_f16:
                    if (imm16_is(inst, z, 0)) {
                        inst[i].op = umbra_mul_f16; inst[i].z = 0;
                    } else if (imm16_is(inst, x, 0x3C00)) {
                        inst[i].op = umbra_add_f16;
                        inst[i].x = y; inst[i].y = z; inst[i].z = 0;
                    } else if (imm16_is(inst, y, 0x3C00)) {
                        inst[i].op = umbra_add_f16; inst[i].y = z; inst[i].z = 0;
                    } else { reduced = 0; }
                    break;
            }
            ...
            if (!reduced
                    && (inst[i].op == umbra_add_f32
                     || inst[i].op == umbra_add_f16)) {
                Op fma_op, mul_op;
                if (inst[i].op == umbra_add_f32) {
                    fma_op = umbra_fma_f32;
                    mul_op = umbra_mul_f32;
                } else {
                    fma_op = umbra_fma_f16;
                    mul_op = umbra_mul_f16;
                }
...
static Fn const fn[] = {
    [umbra_add_f16] =  add_f16, [umbra_sub_f16] =  sub_f16,
    [umbra_mul_f16] =  mul_f16, [umbra_div_f16] =  div_f16,
    [umbra_min_f16] =  min_f16, [umbra_max_f16] =  max_f16,
    [umbra_sqrt_f16] = sqrt_f16, [umbra_fma_f16] = fma_f16,

    [umbra_add_f32] =  add_f32, [umbra_sub_f32] =  sub_f32,
    [umbra_mul_f32] =  mul_f32, [umbra_div_f32] =  div_f32,
    [umbra_min_f32] =  min_f32, [umbra_max_f32] =  max_f32,
    [umbra_sqrt_f32] = sqrt_f32, [umbra_fma_f32] = fma_f32,

    [umbra_add_i16] = add_i16, [umbra_sub_i16] = sub_i16,
    [umbra_mul_i16] = mul_i16,
    [umbra_shl_i16] = shl_i16, [umbra_shr_u16] = shr_u16,
    [umbra_shr_s16] = shr_s16,
    [umbra_and_16] = and_16, [umbra_or_16]  =  or_16,
    [umbra_xor_16] = xor_16, [umbra_sel_16] = sel_16,

    [umbra_add_i32] = add_i32, [umbra_sub_i32] = sub_i32,
    [umbra_mul_i32] = mul_i32,
    [umbra_shl_i32] = shl_i32, [umbra_shr_u32] = shr_u32,
    [umbra_shr_s32] = shr_s32,
    [umbra_and_32] = and_32, [umbra_or_32]  =  or_32,
    [umbra_xor_32] = xor_32, [umbra_sel_32] = sel_32,

    [umbra_f32_from_i32] = f32_from_i32,
    [umbra_i32_from_f32] = i32_from_f32,
    [umbra_f16_from_f32] = f16_from_f32,
    [umbra_f32_from_f16] = f32_from_f16,

    [umbra_eq_f16] = eq_f16, [umbra_ne_f16] = ne_f16,
    [umbra_lt_f16] = lt_f16, [umbra_le_f16] = le_f16,
    [umbra_gt_f16] = gt_f16, [umbra_ge_f16] = ge_f16,

    [umbra_eq_f32] = eq_f32, [umbra_ne_f32] = ne_f32,
    [umbra_lt_f32] = lt_f32, [umbra_le_f32] = le_f32,
    [umbra_gt_f32] = gt_f32, [umbra_ge_f32] = ge_f32,

    [umbra_eq_i16] = eq_i16, [umbra_ne_i16] = ne_i16,
    [umbra_lt_s16] = lt_s16, [umbra_le_s16] = le_s16,
    [umbra_gt_s16] = gt_s16, [umbra_ge_s16] = ge_s16,
    [umbra_lt_u16] = lt_u16, [umbra_le_u16] = le_u16,
    [umbra_gt_u16] = gt_u16, [umbra_ge_u16] = ge_u16,

    [umbra_eq_i32] = eq_i32, [umbra_ne_i32] = ne_i32,
    [umbra_lt_s32] = lt_s32, [umbra_le_s32] = le_s32,
    [umbra_gt_s32] = gt_s32, [umbra_ge_s32] = ge_s32,
    [umbra_lt_u32] = lt_u32, [umbra_le_u32] = le_u32,
    [umbra_gt_u32] = gt_u32, [umbra_ge_u32] = ge_u32,
};
...
struct umbra_interpreter* umbra_interpreter(struct umbra_inst const inst[], int insts) {
    ...
    for (int i = 0; i < insts; i++) {
        ...
        int const X = ci(inst[i].x)-c,
                  ...
                  Z = ci(inst[i].z)-c;
        switch (inst[i].op) {
            case umbra_imm_16: emit(.fn=imm_16, .x=inst[i].immi); break;
            case umbra_imm_32: emit(.fn=imm_32, .x=inst[i].immi); break;
            case umbra_lane:   emit(.fn=lane);                     break;
